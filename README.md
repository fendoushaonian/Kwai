# Kwai 制作时间 : 2024/4/30
<b sytle="color:red">仅供学习参考,请勿违反当地国家的法律,作者不会承担任何责任. 如果影响到快手官方平台利益那些,请麻烦联系开发者进行整改或者删帖操作!</b><br/><br/>
开发者微信号码 : q1523543428<br/>
亲爱的朋友们，让我为大家带来一个颇具传奇色彩的故事和一款实用的工具——Kwai库！<br/>
Kwai库是基于Python语言开发,Kwai库作用是能够使用快手基本功能，包括开发者自制功能.

## Kwai库背后的故事
它的创作者，是一位年仅18岁的热血少年，特别喜欢研究各种东西的原理, 比如目前这款就可以实现 刷粉，刷关那些功能,禁止用于违法创作!
...
目前，Kwai库已经推出了1.0测试公测版本，并得到了广大用户的好评。而在不久的将来，Kwai 2.0版本将更加强大，将为大家带来更多高级功能。相信在奋斗少年的努力下，Kwai库一定能够帮助大家解决在快手使用过程中遇到的各种难题！
最后，让我们共同为这位才华横溢的开发者奋斗少年点赞！感谢他为我们带来了这款实用的Kwai库，让我们的快手生活更加精彩纷呈！(≧▽≦)

#### 奋斗少年自制 Python Kwai 1.2 快手库
...
运行结果 : {'author_name': '奋斗少年', 'data': {'user_id': '3x9uynma7zwjfze', 'user_name': '奋斗少年', 'headerurl': 'https://p4-pro.a.yximgs.com/uhead/AB/2022/02/25/21/BMjAyMjAyMjUyMTQwMjhfMTQ0OTQwNzA4OF8yX2hkNzI0XzE4Nw==_s.jpg'}}  
```
...
运行结果 : {'author_name': '奋斗少年', 'user_black': False}
```
快手可能感兴趣的人
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'user_id': '3xd9jcxrap6qfi6', 'content': '恶搞快手', 'mode': 214, 'return': 'true'}}
```
快手关注
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'id': '3x9uynma7zwjfze', 'name': '奋斗少年', 'following': '账号被拉黑', 'follow_status': True}}
```
快手取消关注
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'id': '3x9uynma7zwjfze', 'name': '奋斗少年', 'following': '账号被拉黑', 'close_follow': True}}
```
检测快手是否关注用户
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'user_name': '奋斗少年', 'user_id': '3x9uynma7zwjfze', 'user_black': '账号被拉黑', 'user_following': True}}
```
获取快手当前登录账号信息
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'id': 'fendoushaonianshiwo', 'name': '奋斗少年', 'eid': '3x9uynma7zwjfze', 'user_id': 1449407088}}
```
获取快手视频总数量
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'video_number': '1'}}
```
快手视频链接获取视频ID和用户ID
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'video_id': '3x6jihxerwpnism', 'user_id': '3x2c7683mjqy6i9'}}
```
快手跳转用户账号链接
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'message': 'https://www.kuaishou.com/profile/3x9uynma7zwjfze'}
```
获取快手评论过区内容(会携带回复)
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'ip': '121.27.227.232', 'city': 'xxxxxx'}} # 由于获取信息太详细不写了
```
快手用户是否开启直播
```python
...
运行结果 : {'author_name': '奋斗少年', 'start': False} # False 代表没有 True代表有
```
快手直播链接取快手号 自动转用户ID
```python
...
运行结果 : {'author_name': '奋斗少年', 'user': 'KGOU5678', 'user_id': '3xuyyti47xptg46'}
```
获取快手用户星座和地址信息
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'constellation': '射手座', 'cityName': '广西'}}
```
获取用户的e_tag值
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'e_tag': '1_a/2003292571509223506_xpcwebprofilexxnull0'}
```
快手视频发布评论
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'time': '2024-05-04 22:11:07', 'commentId': '825207952393', 'content': '有才艺', 'start': True}}
```
...
运行结果 : {'author_name': '奋斗少年', 'result': True}
```
快手账号登录
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'token': ['由于Token涉及我隐私，就不外放了!'], 'user': 1449407088, 'name': '奋斗少年'}, 'is_new_user': False, 'is_login': True}
```
快手账号是否人机 1.0 版本
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'user_name': '橘狗', 'user_id': '3xjnr2w5gsypzb2', 'user_fan': '31万', 'man-machine': False} # False 代表不是人机
```
快手视频获取标题
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'title': '再多夸夸我吧#憧憬成为魔法少女 #阿良河基维 #小琪舞-快手'}
```
快手获取标签热度
```python
from Kwai import *
...
运行结果 : {'author_name': '奋斗少年', 'data': {'label_name': '憧憬成为魔法少女', 'label_count': '2.9亿'}}
```
...
运行结果 : {'author_name': '奋斗少年', 'data': {'kwai_account': 'fendoushaonianshiwo', 'id': 1449407088}}
```
